...
u8 lwip_comm_mem_malloc(void)
{
	...
	/*
	printf( "lwip_comm - mymalloc: %d, %d, %d, %d.\r\n",
	         mempsize, ramheapsize, TCPIP_THREAD_STACKSIZE*4, LWIP_DHCP_STK_SIZE*4 );*/
...
void Lwip_SetIPAddr(u8 a, u8 b, u8 c, u8 d)
{
	lwipdev.ip[0] = a;	
	lwipdev.ip[1] = b;
	lwipdev.ip[2] = c;
...
u8 Lwip_CommInit(void)
{
	...
	if(ETH_Mem_Malloc())
	{
		return ETH_MEM_MALLOC_ERROR;		//内存申请失败
	}

	if(lwip_comm_mem_malloc())
	{
		return LWIP_MEM_MALLOC_ERROR;	    //内存申请失败
	}

#ifdef MV88E6061_ENABLE
	if(MV88E6061_Init())
#endif
	{
		return LAN8720_MAC_ERROR;		//初始化LAN8720失败 
	}

	tcpip_init(NULL,NULL);				//初始化tcp ip内核,该函数里面会创建tcpip_thread内核任务
    lwip_comm_default_ip_set(&lwipdev);	//设置默认IP等信息 -- 存入Flash 上电初始化读取
#if LWIP_DHCP		//使用动态IP
	ipaddr.addr = 0;
	netmask.addr = 0;
	gw.addr = 0;
